...
# import tqdm
# from log.log import log_epoch
# from modules.utils import Averager

# def val(model, val_iter, criterion, device, converter, epoch):
#     total_num = 0.
#     total_loss = 0.
#     total_acc = 0.
...
#                 loss = criterion(preds.view(-1, preds.shape[-1]), target.contiguous().view(-1)).items()
...
#                 #         acc_num += 1
#                 loss_avg=Averager()
#                 val_loss,acc_num= loss_avg.add(loss)
#                 total_loss += val_loss
#                 total_acc += acc_num
...
import tqdm
from log.log import log_epoch
from log.log import log_epoch_val


def val_DR(crnn, val_iter, criterion, device, converter, epoch):
    total_num = 0.
    total_loss = 0.
    total_acc = 0.
    with tqdm.tqdm(range(len(val_iter))) as tbar:
        for i in tbar:
            ...
            with torch.no_grad():
                preds = crnn(images, text).permute(1, 0, 2) 
                preds_size = torch.IntTensor([preds.size(0)] * batch_size)
            ...
            tbar.set_description('val  {}'.format(epoch))
            tbar.set_postfix(loss=loss.item() / batch_size, acc=acc_num / batch_size)
            tbar.update()
            
    log_epoch_val(total_loss / total_num, total_acc / total_num, 'val')
    return total_loss / total_num, total_acc / total_num